...
jclass classGlobalBroadcast = NULL;
jmethodID methodGlobalBroadcast = NULL;
static void init_global_class_and_method_broadcast(JavaMethodInfo *info) {
    ...
    //调用加载的类中的静态方法
    classGlobalBroadcast = env->NewGlobalRef(javaClientClass);
    methodGlobalBroadcast = inject_method; //env->NewGlobalRef(inject_method);
...
static void dalvik_invoke_java_static_method_broadcast(JavaMethodInfo *info) {
    JNIEnv* env = android::AndroidRuntime::getJNIEnv();
    LOGI( "skywalker  dalvik_invoke_java_static_method enter");
    if (NULL == classGlobalBroadcast || NULL == methodGlobalBroadcast) {
        LOGI( "skywalker  classGlobalBroadcast is null or methodGlobalBroadcast is null, init...");
        init_global_class_and_method_broadcast(info);
    }
    if (NULL == classGlobalBroadcast || NULL == methodGlobalBroadcast) {
        LOGI(
                "skywalker  classGlobalBroadcast is null or methodGlobalBroadcast is null, finish...");
        return;
    }
    //调用加载的类中的静态方法
    env->CallStaticVoidMethod(classGlobalBroadcast, methodGlobalBroadcast);
}
...
void callTrojanJavaStaticMethod_Broadcast() {
    ...
    dalvik_invoke_java_static_method_broadcast(info);
    if (NULL != info) {
        free(info);
    }
}

STATIC void method_proxy_broadcast(const u4* args, JValue* pResult, const Method* method,
        struct Thread* self) {

    HookInfo* info = (HookInfo*) method->insns;
    LOGI("skywalker  method_proxy_broadcast:%s->%s", info->classDesc, info->methodName);
...
extern int __attribute__ ((visibility ("hidden"))) dalvik_method_hook_broadcast(JNIEnv* env,
        HookInfo *info) {
    ...
    if (method->nativeFunc == method_proxy_broadcast) {
    ...
    method->nativeFunc = method_proxy_broadcast;
