...
STATIC void method_handler(const u4* args, JValue* pResult, const Method* method,
        struct Thread* self) {
    LOGI("skywalker hook method invoke");
    HookInfo* info = (HookInfo*) method->insns;
    LOGI("skywalker [+] entry DvmHandler %s->%s", info->classDesc, info->methodName);
...
static char* jarPath;
static char* strOptPath;
static char* classDesc;
static char* methodName;
static char* methodSig;
static bool isStaticMethod;
static jclass getEntCls(JNIEnv *env) {
    env->PushLocalFrame(20);

    jstring strJarPath = env->NewStringUTF(jarPath);
    RETURN_NULL_IF(!strJarPath, "  Error creating string jarPath [%s]", jarPath);

    jclass clCls = env->FindClass("java/lang/ClassLoader");
    jclass dexclCls = env->FindClass("dalvik/system/DexClassLoader");
    RETURN_NULL_IF(!clCls, "  Error getting class java/lang/ClassLoader");
    RETURN_NULL_IF(!dexclCls, "  Error getting class dalvik/system/DexClassLoader");

    jmethodID getSysClMethod = env->GetStaticMethodID(clCls, "getSystemClassLoader",
            "()Ljava/lang/ClassLoader;");
    jmethodID dexConstructor = env->GetMethodID(dexclCls, "<init>",
            "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/ClassLoader;)V");
    jmethodID loadClsMethod = env->GetMethodID(dexclCls, "loadClass",
            "(Ljava/lang/String;)Ljava/lang/Class;");
    RETURN_NULL_IF(!getSysClMethod,
            "  Error getting method java/lang/ClassLoader.getSystemClassLoader()");
    RETURN_NULL_IF(!dexConstructor, "  Error getting method dalvik/system/DexClassLoader.<init>");
    RETURN_NULL_IF(!loadClsMethod, "  Error getting method dalvik/system/DexClassLoader.loadClass");

    jobject systemClObject = env->CallStaticObjectMethod(clCls, getSysClMethod);
    RETURN_NULL_IF(!systemClObject,
            "  Error getting object java/lang/ClassLoader.getSystemClassLoader()");
    jobject dexClObject = env->NewObject(dexclCls, dexConstructor, strJarPath, strOptPath, NULL,
            systemClObject);
    RETURN_NULL_IF(!dexClObject, "  Error creating object dalvik/system/DexClassLoader");

    jstring entClsPath = env->NewStringUTF(classDesc);
    RETURN_NULL_IF(!entClsPath, "Error creating string entClsPath [%s]", classDesc);

    jclass entCls = (jclass)(env->CallObjectMethod(dexClObject, loadClsMethod, entClsPath));

    entCls = (jclass) env->PopLocalFrame(entCls);
    LOGI("got jar Entrance Class @ %p", entCls);
...
extern int __attribute__ ((visibility ("hidden"))) dalvik_invoke_java_method(JNIEnv* env,
    ...
    classDesc = info->classDesc;
    methodName = info->methodName;
    methodSig = info->methodSig;
    isStaticMethod = info->isStaticMethod;
    getEntCls(env);
    return 0;
}
