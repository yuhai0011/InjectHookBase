 ...
 */
static void jni_invoke_java_method() {
	LOGI("skywalker jni_invoke_java_method enter");
	JNIEnv* env = android::AndroidRuntime::getJNIEnv();

	jstring dexpath = env->NewStringUTF("/data/system/InjectHookTrojan.apk");
	jstring dex_odex_path = env->NewStringUTF("/data/dalvik-cache/");
	jstring javaClassName = env->NewStringUTF(	"com/futureagent/injecthooktrojan/HookUtils");
	const char* func = "hookMethod";
	...
	//获取加载的类中的方法
	jmethodID inject_method = env->GetStaticMethodID(javaClientClass, func,
			"()V");
	//调用加载的类中的静态方法
	env->CallStaticVoidMethod(javaClientClass, inject_method);
}

void callTrojanJavaStaticMethod() {
	jni_invoke_java_method();
}
...
STATIC void method_handler(const u4* args, JValue* pResult,
	...
	callTrojanJavaStaticMethod();

	HookInfo* info = (HookInfo*) method->insns;
	LOGI(
			"skywalker [+] entry DvmHandler %s->%s", info->classDesc, info->methodName);
...
extern int __attribute__ ((visibility ("hidden"))) dalvik_java_method_hook(
	...
	if (method->nativeFunc == method_handler) {
		LOGW(
				"skywalker  [*] %s->%s method had been hooked", classDesc, methodName);
		return -1;
	}
...
extern int __attribute__ ((visibility ("hidden"))) dalvik_invoke_java_method(
		JNIEnv* env, JavaMethodInfo *info) {
